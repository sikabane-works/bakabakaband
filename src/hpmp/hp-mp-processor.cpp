    ...
    std::function<PERCENTAGE(player_type *)> damage_rate, std::function<void(player_type *, int)> additional_effect)
{
    ...
    int damage = 0;
    bool resist_levitation = (creature_ptr->levitation && !f_ptr->flags.has(FF::CHAOS_TAINTED) && !f_ptr->flags.has(FF::VOID));
...
void process_player_hp_mp(player_type *creature_ptr)
{
    ...
    if (f_ptr->flags.has(FF::DUNG_POOL) && !is_invuln(creature_ptr)) {
