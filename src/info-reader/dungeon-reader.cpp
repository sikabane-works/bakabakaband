...
static bool grab_one_basic_monster_flag(dungeon_type *d_ptr, std::string_view what)
{
    ...
    /*
    if (EnumClassFlagGroup<MonsterSpecialType>::grab_one_flag(d_ptr->mon_special_flags, , what)) {
        return true;
    }
    */

    if (EnumClassFlagGroup<MonsterMiscType>::grab_one_flag(d_ptr->mon_misc_flags, r_info_flags1, what)) {
        return true;
    }
