...
void exe_movement(PlayerType *player_ptr, DIRECTION dir, bool do_pickup, bool break_trap)
{
    ...
    p_can_kill_walls &= f_ptr->flags.has(FloorFeatureType::HURT_DISI);
    p_can_kill_walls &= !p_can_enter || f_ptr->flags.has_not(FloorFeatureType::LOS);
    p_can_kill_walls &= f_ptr->flags.has_not(FloorFeatureType::PERMANENT);
