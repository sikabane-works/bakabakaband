...
void build_stores(PlayerType *player_ptr, const Pos2D &pos_ug, const std::vector<UndergroundBuilding> &underground_buildings)
{
    for (const auto &ug_building : underground_buildings) {
        const auto y1 = pos_ug.y + ug_building.get_north_west().y - 5;
        const auto x1 = pos_ug.x + ug_building.get_north_west().x - 5;
        const auto y2 = pos_ug.y + ug_building.get_south_east().y + 5;
        const auto x2 = pos_ug.x + ug_building.get_south_east().x + 5;
