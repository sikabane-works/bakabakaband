...
void build_stores(PlayerType *player_ptr, const Pos2D &pos_ug, const std::vector<UndergroundBuilding> &underground_buildings)
{
    for (const auto &ug_building : underground_buildings) {
        generate_room_floor(player_ptr, pos_ug.y + ug_building.north_west.y - 5, pos_ug.x + ug_building.north_west.x - 5, pos_ug.y + ug_building.south_east.y + 5, pos_ug.x + ug_building.south_east.x + 5, false);
    }
