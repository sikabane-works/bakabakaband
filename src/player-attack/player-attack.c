...
static void get_bare_knuckle_exp(player_type *attacker_ptr, player_attack_type *pa_ptr)
{
    ...
    if ((r_ptr->level + 10) <= attacker_ptr->lev || (attacker_ptr->skill_exp[SKILL_SUDE] >= s_info[attacker_ptr->pclass].s_max[SKILL_SUDE]))
        return;

    if (attacker_ptr->skill_exp[SKILL_SUDE] < WEAPON_EXP_BEGINNER)
        attacker_ptr->skill_exp[SKILL_SUDE] += 40;
    else if ((attacker_ptr->skill_exp[SKILL_SUDE] < WEAPON_EXP_SKILLED))
        attacker_ptr->skill_exp[SKILL_SUDE] += 5;
    else if ((attacker_ptr->skill_exp[SKILL_SUDE] < WEAPON_EXP_EXPERT) && (attacker_ptr->lev > 19))
        attacker_ptr->skill_exp[SKILL_SUDE] += 1;
