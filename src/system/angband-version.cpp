...
std::string get_version()
{
    ...
    if (VERSION_STATUS == VersionStatusType::RELEASE) {
        ...
        return format(_("馬鹿馬鹿蛮怒 %d.%d.%d.%d(%s)", "Hengband %d.%d.%d.%d(%s)"), H_VER_MAJOR, H_VER_MINOR, H_VER_PATCH, H_VER_EXTRA, mode);
    }

    return format(_("馬鹿馬鹿蛮怒 %d.%d.%d%s%d", "Hengband %d.%d.%d%s%d"), H_VER_MAJOR, H_VER_MINOR, H_VER_PATCH, expr.data(), H_VER_EXTRA);
}
