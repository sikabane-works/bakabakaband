 ...
 * 今のところは毒と同じ
 */
process_result effect_monster_dirt(player_type *player_ptr, effect_monster_type *em_ptr)
{
    if (em_ptr->seen)
        em_ptr->obvious = true;
...
process_result effect_monster_stungun(player_type *player_ptr, effect_monster_type *em_ptr)
{
    if (em_ptr->seen)
        em_ptr->obvious = true;
