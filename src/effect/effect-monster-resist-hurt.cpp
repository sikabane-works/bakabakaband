 ...
 * 今のところは毒と同じ
 */
ProcessResult effect_monster_dirt(PlayerType *player_ptr, effect_monster_type *em_ptr)
{
    if (em_ptr->seen) {
        em_ptr->obvious = true;
    }
...
ProcessResult effect_monster_stungun(PlayerType *player_ptr, effect_monster_type *em_ptr)
{
    if (em_ptr->seen) {
        em_ptr->obvious = true;
    }
