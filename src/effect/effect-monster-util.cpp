 ...
 */
static void substitute_effect_monster(effect_monster_type *em_ptr, MONSTER_IDX who, POSITION r, POSITION y, POSITION x, HIT_POINT dam, AttributeType effect_type, BIT_FLAGS flag, bool see_s_msg)
{
	em_ptr->who = who;
	em_ptr->r = r;
	em_ptr->y = y;
 ...
 * @param see_s_msg TRUEならばメッセージを表示する
 */
effect_monster_type *initialize_effect_monster(PlayerType *player_ptr, effect_monster_type *em_ptr, MONSTER_IDX who, POSITION r, POSITION y, POSITION x, HIT_POINT dam, AttributeType effect_type, BIT_FLAGS flag, bool see_s_msg)
{
	...
	em_ptr->m_player_ptr = (em_ptr->who > 0) ? &floor_ptr->m_list[em_ptr->who] : nullptr;
	...
	em_ptr->caster_lev = (em_ptr->who > 0) ? r_info[em_ptr->m_player_ptr->r_idx].level : (player_ptr->lev * 2);
	return em_ptr;
}
