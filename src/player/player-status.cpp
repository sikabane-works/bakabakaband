...
static void update_max_mana(PlayerType *player_ptr)
{
    ...
    if (any_bits(mp_ptr->spell_xtra, extra_magic_glove_reduce_mana)) {
        ...
        if (o_ptr->bi_id && flags.has_not(TR_FREE_ACT) && flags.has_not(TR_DEC_MANA) && flags.has_not(TR_EASY_SPELL) && !((flags.has(TR_MAGIC_MASTERY)) && (o_ptr->pval > 0)) && !((flags.has(TR_DEX)) && (o_ptr->pval > 0))) {
...
static short calc_to_damage(PlayerType *player_ptr, INVENTORY_IDX slot, bool is_real_value)
{
    ...
    PlayerClass pc(player_ptr);
    if (pc.equals(PlayerClassType::PRIEST) && (flags.has_not(TR_BLESSED)) && ((o_ptr->tval == ItemKindType::SWORD) || (o_ptr->tval == ItemKindType::POLEARM))) {
        damage -= 2;
    } else if (pc.equals(PlayerClassType::BERSERKER)) {
