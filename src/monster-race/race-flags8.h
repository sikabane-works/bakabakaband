#pragma once

enum race_flags8 : uint32_t {
    RF8_WILD_ONLY = 0x00000001,
    RF8_WILD_TOWN = 0x00000002,
    RF8_NO_QUEST = 0x00000004,
    RF8_WILD_SHORE = 0x00000008,
    RF8_WILD_OCEAN = 0x00000010,
    RF8_WILD_WASTE = 0x00000020,
    RF8_WILD_WOOD = 0x00000040,
    RF8_WILD_VOLCANO = 0x00000080,
    RF8_SPIDER = 0x00000100,
    RF8_WILD_MOUNTAIN = 0x00000200,
    RF8_WILD_GRASS = 0x00000400,
    RF8_WARRIOR = 0x0000800,
    RF8_ROGUE = 0x00001000,
    RF8_PRIEST = 0x0002000,
    RF8_MAGE = 0x00004000,
    RF8_PALADIN = 0x00008000,
    RF8_RANGER = 0x00010000,
    RF8_SAMURAI = 0x00020000,
    RF8_ELF = 0x00040000,
    RF8_DWARF = 0x00080000,
    RF8_HOBBIT = 0x00100000,
    RF8_QUYLTHLUG = 0x00200000,
    RF8_ELDRAZI = 0x00400000,
    RF8_KARATEKA = 0x00800000,
    RF8_NINJA = 0x01000000,
    RF8_SUMOU_WRESTLER = 0x02000000,
    RF8_YAKUZA = 0x04000000,
    RF8_JOKE = 0x08000000,
    RF8_HOMO_SEXUAL = 0x10000000,
    RF8_NASTY = 0x20000000,
    RF8_WILD_SWAMP = 0x40000000,
    RF8_WILD_ALL = 0x80000000,
};
